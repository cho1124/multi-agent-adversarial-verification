...
```mermaid
flowchart LR
    E["🎯 Executor<br/>(테제)<br/>설계 제안"] -->|설계안 제출| C["⚔️ Challenger<br/>(안티테제)<br/>모순 공격"]
    C -->|반박 제출| A["⚖️ Arbiter<br/>(진테제)<br/>타당성 판정"]
    A -->|"VALID → 설계 수정 요구"| E
    A -->|"INVALID → 반박 기각"| C
    A -->|"종료 조건 충족"| R["📋 최종 보고서"]
...
```mermaid
flowchart TB
    subgraph Phase0["Phase 0: 초기화"]
        P0["주제 분석 → 소주제 분해"]
    end

    subgraph Phase1["Phase 1: 적대적 검증 루프"]
        direction LR
        EX["Executor<br/>설계 제안/수정"] --> CH["Challenger<br/>모순 탐색"]
        CH --> AR["Arbiter<br/>VALID / WEAK / INVALID"]
        AR -->|"미해소 모순 존재"| EX
    end

    subgraph Phase2["Phase 2: 수렴 판정"]
        P2["3연속 INVALID 또는<br/>max 라운드 도달 → 종료"]
    end

    subgraph Phase3["Phase 3: 최종 보고서"]
        P3["발견된 모순 · 개선안 · 미해소 이슈 정리"]
    end
